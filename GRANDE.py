...
class GRANDE(tf.keras.Model):
    ...
    def call(self, inputs, training):
        ...
        if self.output_layer.data_subset_fraction < 1.0 and training:
            result = tf.scatter_nd(indices=tf.expand_dims(self.output_layer.data_select, 2),
                                   updates=tf.transpose(output), shape=[tf.shape(inputs)[0]])
    ...
    def fit(self,
        ...
        self.compile(loss=self.loss_name, metrics=self.metrics_name, mean=self.mean, std=self.std,
        ...
        if X_val is not None and y_val is not None:
            ...
            validation_data = tf.data.Dataset.from_tensor_slices(
                (tf.dtypes.cast(tf.convert_to_tensor(validation_data[0]), tf.float32),
    ...
    def compile(self,
        ...
        if self.objective == 'classification':
            ...
            if loss == 'crossentropy':
                if not self.focal_loss:
                    loss_function = tf.keras.losses.SparseCategoricalCrossentropy(
                        from_logits=self.from_logits)  # tf.keras.losses.get('categorical_crossentropy')
                else:
                    loss_function = SparseCategoricalFocalLoss(gamma=2, class_weight=self.class_weights,
            ...
            if loss == 'crossentropy':
                if not self.focal_loss:
                    loss_function = tf.keras.losses.BinaryCrossentropy(
                        from_logits=self.from_logits)  # tf.keras.losses.get('binary_crossentropy')
                else:
                    loss_function = tf.keras.losses.BinaryFocalCrossentropy(alpha=0.5, gamma=2,
            ...
            loss_function = loss_function_regression(loss_name=loss, mean=kwargs['mean'], std=kwargs['std'],
                                                     transformation_type='mean')

        self.weights_optimizer = get_optimizer_by_name(optimizer_name=self.optimizer_name,
                                                       learning_rate=self.learning_rate_weights, warmup_steps=0,
                                                       cosine_decay_steps=self.cosine_decay_steps)
        self.index_optimizer = get_optimizer_by_name(optimizer_name=self.optimizer_name,
                                                     learning_rate=self.learning_rate_index, warmup_steps=0,
                                                     cosine_decay_steps=self.cosine_decay_steps)
        self.values_optimizer = get_optimizer_by_name(optimizer_name=self.optimizer_name,
                                                      learning_rate=self.learning_rate_values, warmup_steps=0,
                                                      cosine_decay_steps=self.cosine_decay_steps)
        self.leaf_optimizer = get_optimizer_by_name(optimizer_name=self.optimizer_name,
                                                    learning_rate=self.learning_rate_leaf, warmup_steps=0,
                                                    cosine_decay_steps=self.cosine_decay_steps)
...
class GRANDEBlock(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        if self.data_subset_fraction < 1.0:
            ...
            if self.bootstrap:
                self.data_select = tf.random.uniform(shape=(self.n_estimators, self.subset_size), minval=0,
                                                     maxval=self.batch_size, dtype=tf.int32)
            else:

                indices = [np.random.choice(self.batch_size, size=self.subset_size, replace=False) for _ in
        ...
        if self.n_estimators > 1:
            self.features_by_estimator = tf.stack(
                [np.random.choice(self.number_of_variables, size=(self.selected_variables), replace=False, p=None) for _
                 in range(self.n_estimators)])
        else:
            self.features_by_estimator = tf.constant(
                [np.random.choice(self.number_of_variables, size=(self.selected_variables), replace=False, p=None)])
        ...
        for leaf_index in tf.unstack(tf.constant([i for i in range(self.leaf_node_num_)])):
            for current_depth in tf.unstack(tf.constant([i for i in range(1, self.depth + 1)])):
                path_identifier = tf.cast(
                    tf.math.floormod(tf.math.floor(leaf_index / (tf.math.pow(2, (self.depth - current_depth)))), 2),
                    tf.float32)
                internal_node_index = tf.cast(tf.cast(tf.math.pow(2, (current_depth - 1)), tf.float32) + tf.cast(
                    tf.math.floor(leaf_index / (tf.math.pow(2, (self.depth - (current_depth - 1))))), tf.float32) - 1.0,
                                              tf.int64)
        ...
        self.internal_node_index_list = tf.reshape(tf.cast(tf.stack(self.internal_node_index_list), tf.int64),
                                                   (-1, self.depth))

        leaf_classes_array_shape = [self.n_estimators,
                                    self.leaf_node_num_, ] if self.objective == 'binary' or self.objective == 'regression' else [
            self.n_estimators, self.leaf_node_num_, self.number_of_classes]

        weight_shape = [self.n_estimators, self.leaf_node_num_]

        self.estimator_weights = self.add_weight(name="estimator_weights",
                                                 initializer=tf.keras.initializers.get({'class_name': self.initializer,
                                                                                        'config': {
        ...
        self.split_index_array = self.add_weight(name="split_index_array",
                                                 initializer=tf.keras.initializers.get({'class_name': self.initializer,
                                                                                        'config': {
                                                                                            'seed': self.random_seed + 3}}),
                                                 trainable=True,
                                                 shape=[self.n_estimators, self.internal_node_num_,
                                                        self.selected_variables])
        self.leaf_classes_array = self.add_weight(name="leaf_classes_array",
                                                  initializer=tf.keras.initializers.get({'class_name': self.initializer,
                                                                                         'config': {
                                                                                             'seed': self.random_seed + 4}}),
                                                  trainable=True,
    ...
    def call(self, inputs, training, output_weights=False):
        ...
        if self.data_subset_fraction < 1.0 and training:
            ...
            X_estimator = tf.transpose(
                tf.gather_nd(tf.transpose(X_estimator, [0, 2, 1]), tf.expand_dims(self.data_select, 2), batch_dims=1),
                [1, 0, 2])
        else:
            # use complete data
        ...
        p = tf.reduce_prod(((1 - self.path_identifier_list) * node_result_corrected + self.path_identifier_list * (
                    1 - node_result_corrected)), axis=3)
...
def get_optimizer_by_name(optimizer_name, learning_rate, warmup_steps, cosine_decay_steps):
    ...
    if optimizer_name == 'SWA':
        optimizer = tfa.optimizers.SWA(optimizer=tf.keras.optimizers.legacy.Adam(learning_rate=learning_rate),
                                       average_period=5)
    elif optimizer_name == 'GradientAccumulator':
        optimizer = GradientAccumulateOptimizer(accum_steps=10,
